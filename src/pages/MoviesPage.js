...
const Movies = () => {
  ...
  const [isSearchActive, setIsSearchActive] = useState(false); // Tracks search state
  const observerRef = useRef(null);
  const navigate = useNavigate();
  const moviesPerPage = 12; // Number of movies to load per page
  ...
  useEffect(() => {
    if (!isSearchActive) {
      // Fetch movies data from the backend when not in search mode
      const fetchMovies = () => {
        let endpoint = 'http://localhost:5000/movies';
        const params = {};
  ...
  }, [filterBy, filterValue, isSearchActive]);

  useEffect(() => {
    // Load initial movies when the movies list is updated
    loadMoreMovies();
  }, [movies]);
  ...
  const handleSearch = () => {
    ...
    axios
      .get('http://localhost:5000/movies/search', { params: { query: searchQuery } })
      .then((response) => {
        setMovies(response.data);
        setVisibleMovies([]);
      ...
      <div style={styles.mainContent}>
        ...
        <div style={styles.searchContainer}>
          ...
          <button
            ...
            onClick={isSearchActive ? clearSearch : handleSearch}
          >
